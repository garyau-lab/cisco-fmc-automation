...
def pre_check(interface):
    ...
    try:
        sec_zone = response['securityZone']
        return ("exist")
    except:
       return
...
print_header('Configure Interface Security Zone')
for interface in intf_dict:
    if interface['interface_type'] == "PhysicalInterface":
        print_action(interface['interface_name']+" "+interface['security_zone'])
        if pre_check(interface) == "exist":
            print_skip()
        else:
            creation(interface)
