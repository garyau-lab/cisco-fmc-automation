...
def pre_check(net_obj):
    try:
        response = fmc.object.networkgroup.override.get(container_name=net_obj['object_name'])
        for i in response:
            if (i['overrides']['target']['id']) == ftd_id:
                return ("exist")                
    except:
        return
...
def creation(net_obj,literals):
    obj_id = fmc.object.networkgroup.get(name=net_obj['object_name'])['id']
    payload = {
        "overrides": {
            "parent": {
                "id": obj_id
    ...
    fmc.object.networkgroup.update(data=payload)
...
print_header('Add Entries into Overridable NetworkGroup Object')
for net_obj in object_dict:
    if net_obj['override'] == "yes" and net_obj['object_type'] == "NetworkGroup":
