...
def pre_check(net_obj):
    try:
        response = fmc.object.network.get(name=net_obj['object_name'], override_target_id=ftd_id)
        return ("exist")
    except:
        return
...
def creation(net_obj):
    obj_id = fmc.object.network.get(name=net_obj['object_name'])['id']
    payload = {
        "overrides": {
            "parent": {
                "id": obj_id
    ...
    fmc.object.network.update(data=payload)

print_header('Add Entry into Overridable Network Object')
for net_obj in object_dict:
    if net_obj['override'] == "yes" and net_obj['object_type'] == "Network":
